...
int top_shop = -1;
CMD_DATA *cmd_say, *cmd_tell, *cmd_emote, *cmd_slap, *cmd_puke;
...
int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
  ...
  if (!CAN_SEE(keeper, ch)) {
    ...
    do_say(keeper, actbuf, cmd_say, 0, 0);
    return (FALSE);
  }
  if (IS_GOD(ch))
    return (TRUE);
  ...
  if ((IS_GOOD(ch) && NOTRADE_GOOD(shop_nr)) ||
    ...
    do_tell(keeper, buf, cmd_tell, 0, 0);
    return (FALSE);
  }
  if (IS_NPC(ch))
    return (TRUE);
...
void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
  ...
  if (buy_price(obj, shop_nr, keeper, ch) > GET_GOLD(ch) && !IS_GOD(ch)) {
    ...
    do_tell(keeper, actbuf, cmd_tell, 0, 0);

    switch (SHOP_BROKE_TEMPER(shop_nr)) {
    case 0:
      do_action(keeper, strcpy(actbuf, GET_NAME(ch)), cmd_puke, 0, 0);	/* strcpy: OK (MAX_NAME_LENGTH < MAX_INPUT_LENGTH) */
      return;
    case 1:
      do_echo(keeper, strcpy(actbuf, "smokes on his joint."), cmd_emote, 0, SCMD_EMOTE);	/* strcpy: OK */
      return;
    default:
      return;
    }
  ...
  do_tell(keeper, tempbuf, cmd_tell, 0, 0);
...
struct obj_data *get_selling_obj(struct char_data *ch, char *name, struct char_data *keeper, int shop_nr, int msg)
{
  ...
  if (!(obj = get_obj_in_list_vis(ch, name, NULL, ch->carrying))) {
    if (msg) {
      ...
      do_tell(keeper, tbuf, cmd_tell, 0, 0);
    }
    return (NULL);
  }
...
SPECIAL(shop_keeper)
{
  ...
  if (SHOP_FUNC(shop_nr))	/* Check secondary function */
    if ((SHOP_FUNC(shop_nr)) (ch, me, commandp, argument, 0))
      return (TRUE);
...
int ok_damage_shopkeeper(struct char_data *ch, struct char_data *victim)
{
  ...
  for (sindex = 0; sindex <= top_shop; sindex++)
    if (GET_MOB_RNUM(victim) == SHOP_KEEPER(sindex) && !SHOP_KILL_CHARS(sindex)) {
      ...
      do_tell(victim, buf, cmd_tell, 0, 0);
