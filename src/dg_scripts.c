...
/* FIXME: There's a memory leak somewhere here - This isn't always free()'d - Welcor*/
...
/* see note in dg_scripts.h */
      ...
      if (text_processed(field, subfield, vd, str, slen)) return;
        ...
        if (obj_room(o) != NOWHERE)
          snprintf(str, slen,"%c%ld",UID_CHAR, (long)obj_room(o) + ROOM_ID_BASE);
        else strcpy(str, "");
      ...
      if (text_processed(field, subfield, vd, str, slen)) return;
        ...
        if (room_id != NOWHERE)
          snprintf(str, slen, "%ld", (long) room_id + ROOM_ID_BASE); 
        else
          *str = '\0';
      } 
  ...
  while (*p && (left > 0)) {
    ...
    *buf = '\0';
    ...
    /* so it wasn't double %'s */
      ...
      if (*p == '.') {
        *(p++) = '\0';
        for (field = p; *p && ((*p != '%')||(paren_count > 0)); p++) {
          if (*p=='(') {
            *p = '\0';
            paren_count++;
          } else if (*p==')') {
      ...
      if (cl->original) {
        ...
        if (cl->original && process_if(orig_cmd + 6, go, sc, trig,
          ...
          if (GET_TRIG_LOOPS(trig) >= 100) {
            script_log("SCRIPTERR: Trigger VNum %d has looped 100 times!!!",
              GET_TRIG_VNUM(trig));
            break;
          }
        } else {
